...
struct process_ctrl_block {
    ...
    pid_t actual_pid;
    int page_table[MAX_PROC_MEM]; // Array of indexes pointing to frames in main memory allocated to this process if -1, not in main memory
};
...
struct oss_shm {
    ...
    bool allocated_frames[MAX_MAIN_MEM]; // 256 bit vector each bit representing a main memory frame if free or not
};
